...
class InvertedIndex:
    ...
    def __add_document(self, doc_id, text):
        ...
        token_list2 = text.lower().translate(table).split()
        for token in token_list:
            if not token:
                continue
            self.index[token].add(doc_id)

        self.doc_length[doc_id] = len(token_list2)
    ...
    def __get_avg_doc_length(self) -> float:
        if len(self.doc_length) == 0:
            return 0
        counter = 0 
        for value in self.doc_length.items():
           counter += value[1]
        return counter / len(self.doc_length)
